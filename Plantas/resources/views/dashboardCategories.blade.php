...
<body class="bg-light text-dark">
    <div class="d-flex justify-content-center align-items-center min-vh-100 bg-body-secondary">
            ...
            <div class="row flex-nowrap">
                ...
                <div class="col py-3 mt-5">
                    <div class="table-responsive">
                                        ...
                                        <td>
                                            @if (!$trash)
                                                ...
                                                <button type="button" class="btn btn-sm btn-danger"
                                                    ...
                                                    data-bs-target="#deletePostModal{{ $category->id }}">
                                                    Delete
                                                </button>
                                            @else
                                                <!-- Restore Button -->
                                            ...
                                            <div class="modal fade" id="deletePostModal{{ $category->id }}"
                                                tabindex="-1"
                                                aria-labelledby="deletePostModalLabel{{ $category->id }}"
                                                aria-hidden="true">
                                                <div class="modal-dialog">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <h5 class="modal-title"
                                                                id="deletePostModalLabel{{ $category->id }}">Delete
                                                                Category</h5>
                        ...
                        @if (!$trash)
                            <!-- Create Button -->
                            <a href="{{ route('categories.create') }}" class="btn btn-lg btn-success">Create</a>
                        @endif
                    </div>
                </div>
            </div>
        ...
        <div class="position-absolute bottom-0 end-0 m-5">
            @if (Request::is('dashboardCategories') || Request::is('categoryTrash'))
                @if (!$trash)
                    <div class="btn-group">
                        <a href="{{ route('categories.trash') }}" class="btn btn-primary"><i
                                class="bi bi-trash-fill"></i></a>
                    </div>
                @else
                    <div class="btn-group">
                        <a href="{{ route('dashboardCategories') }}" class="btn btn-primary">Original</a>
                    </div>
                @endif
            @endif
