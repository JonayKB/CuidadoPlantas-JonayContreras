            ...
            <div class="fixed-top top-0 text-end end-0 z-2">
                @auth
                    @if (auth()->user()->roles->contains('name', 'admin'))
                        <a href="{{ url('/dashboard') }}" class="btn btn-outline-secondary fw-semibold mt-3 mr-3">Dashboard</a>
                    @endif

                    @if (auth()->user()->roles->contains('name', 'user'))
                    <a href="{{ route('posts.create') }}" class="fw-semibold"><i class="bi bi-plus-circle-fill align-middle text-dark h2 mt-3 pt-3"></i></a>
                @endif
                ...
                @else
                    ...
                    @if (Route::has('register'))
                        <a href="{{ route('register') }}" class="btn btn-outline-secondary fw-semibold mt-3 mx-3">Register</a>
                    @endif
                @endauth
            </div>
        @endif
